...
public final class MySqlDocumentStatusStore implements DocumentStatusStore {
    private final MySqlJdbcConnectionAccess connectionAccess;
    private final String tableName;
    private final String workspaceId;

    public MySqlDocumentStatusStore(DataSource dataSource, MySqlStorageConfig config) {
        this(dataSource, config, "default");
    }

    public MySqlDocumentStatusStore(DataSource dataSource, MySqlStorageConfig config, String workspaceId) {
        this(MySqlJdbcConnectionAccess.forDataSource(dataSource), config, workspaceId);
    }

    MySqlDocumentStatusStore(MySqlJdbcConnectionAccess connectionAccess, MySqlStorageConfig config, String workspaceId) {
    ...
    public void save(StatusRecord statusRecord) {
        ...
        connectionAccess.withConnection(connection -> {
            try (var statement = connection.prepareStatement(
                ...
                statement.setString(6, MySqlJsonCodec.writeObjectMap(record.metadata()));
    ...
    private static StatusRecord readStatus(ResultSet resultSet) throws SQLException {
        return new StatusRecord(
            ...
            resultSet.getString("metadata") == null ? java.util.Map.of() : MySqlJsonCodec.readObjectMap(resultSet.getString("metadata"))
        );
    }
}
