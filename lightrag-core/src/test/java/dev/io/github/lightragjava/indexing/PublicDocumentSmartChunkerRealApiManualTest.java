...
import io.github.lightragjava.indexing.ParsedDocument;
...
class PublicDocumentSmartChunkerRealApiManualTest {
    ...
    void smartChunkerUsesSemanticBoundariesForGenericDensePublicPdf() {
        ...
        var source = new io.github.lightragjava.types.Document(
            parsed.documentId(),
            parsed.title(),
            parsed.plainText(),
            parsed.metadata()
    ...
    void printsRepresentativeGenericSemanticChunksForInspection() {
        ...
        var tokenCounts = semanticChunks.stream()
            .mapToInt(io.github.lightragjava.types.Chunk::tokenCount)
            .summaryStatistics();
    ...
    void comparesGenericSemanticChunksBetweenMediumAndCoarse() {
        ...
        var mediumStats = mediumChunks.stream().mapToInt(io.github.lightragjava.types.Chunk::tokenCount).summaryStatistics();
        var coarseStats = coarseChunks.stream().mapToInt(io.github.lightragjava.types.Chunk::tokenCount).summaryStatistics();
    ...
    void comparesChineseGenericLegacyPlainTextPathAgainstWeakStructuredPath() {
        ...
        var legacyChunker = new SmartChunker(new io.github.lightragjava.indexing.ChunkingProfile(
            io.github.lightragjava.indexing.DocumentType.GENERIC,
        ...
        var legacyStats = legacyChunks.stream().mapToInt(io.github.lightragjava.types.Chunk::tokenCount).summaryStatistics();
        var improvedStats = improvedChunks.stream().mapToInt(io.github.lightragjava.types.Chunk::tokenCount).summaryStatistics();
    ...
    void comparesChineseGenericAdaptiveChunksAgainstStaticSizing() {
        ...
        var staticStats = staticChunks.stream().mapToInt(io.github.lightragjava.types.Chunk::tokenCount).summaryStatistics();
        var adaptiveStats = adaptiveChunks.stream().mapToInt(io.github.lightragjava.types.Chunk::tokenCount).summaryStatistics();
    ...
    private static List<io.github.lightragjava.types.Chunk> chunkParsedGeneric(ParsedDocument parsed, SmartChunkerConfig config) {
        try {
            var method = SmartChunker.class.getDeclaredMethod(
                "chunkParsedDocument",
                ParsedDocument.class,
                Class.forName("io.github.lightragjava.indexing.DocumentType")
            );
            method.setAccessible(true);
            var documentType = Enum.valueOf(
                (Class<Enum>) Class.forName("io.github.lightragjava.indexing.DocumentType"),
                "GENERIC"
            );
            return (List<io.github.lightragjava.types.Chunk>) method.invoke(new SmartChunker(config), parsed, documentType);
        } catch (ReflectiveOperationException exception) {
            throw new IllegalStateException("Failed to invoke SmartChunker parsed-document path for manual test", exception);
        }
    }

    private static long countNoiseChunks(List<io.github.lightragjava.types.Chunk> chunks) {
        return chunks.stream()
            .map(io.github.lightragjava.types.Chunk::text)
            .map(String::strip)
    ...
    private static boolean anyParsedBlockMatches(ParsedDocument parsed, java.util.function.Predicate<String> predicate) {
        return parsed.blocks().stream()
            .map(io.github.lightragjava.indexing.ParsedBlock::text)
            .anyMatch(predicate);
    }

    private static boolean anyChunkMatches(List<io.github.lightragjava.types.Chunk> chunks, java.util.function.Predicate<String> predicate) {
        return chunks.stream()
            .map(io.github.lightragjava.types.Chunk::text)
            .anyMatch(predicate);
    }
    ...
    private static void printMatchingChunks(
        String prefix,
        List<io.github.lightragjava.types.Chunk> chunks,
    ...
    private static void printRepresentativeChunks(String prefix, List<io.github.lightragjava.types.Chunk> chunks, int limit) {
        chunks.stream()
    ...
    private record ChunkResult(
        ...
        io.github.lightragjava.indexing.ChunkingResult chunkingResult
    ) {
