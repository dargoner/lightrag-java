...
#### Minimal Attribution Output Protocol

为避免让模型直接生成任意 `chunkId`，推荐窗口抽取阶段只输出**窗口内相对引用**，例如：

```json
{
  "relations": [
    {
      "source_entity_name": "订单系统",
      "target_entity_name": "PostgreSQL",
      "type": "依赖",
      "description": "订单系统依赖 PostgreSQL 进行事务存储",
      "weight": 1.0,
      "supporting_chunk_indexes": [0, 1]
    }
  ]
}
```
...
1. `supporting_chunk_indexes` 只能引用 `RefinementWindow.chunks` 的下标。
2. refiner 必须在返回前把 index 转成真实 `supportingChunkIds`。
3. 若模型没有给出 attribution，可由后处理器补齐，但只能使用 deterministic 规则。
4. 默认 **不启用** 规则归因兜底；未能归因时直接丢弃该 relation patch。

为保证 schema 可实现、可 mock、可 fixture，建议先固定窗口抽取的原始响应 DTO：

```java
record WindowEntityCandidate(
    String name,
    String type,
    String description,
    List<String> aliases,
...
record WindowRelationCandidate(
...
record WindowExtractionResponse(
...
1. `supportingChunkIndexes` 缺失、为空或越界：
   - relation candidate 直接丢弃并记录 warning
   - entity candidate 不单独写回，只有在后续被有效 relation 引用时才保留
2. index 去重后按窗口原始顺序升序排列。
3. 任意 index 必须满足：`0 <= index < RefinementWindow.chunks.size()`。
4. 解析阶段先将 `supportingChunkIndexes` 映射成真实 `supportingChunkIds`，再生成：
   - `RefinedEntityPatch`
   - `RefinedRelationPatch`
5. `WindowExtractionResponse` 是组件测试与 golden tests 的稳定 fixture 格式，不直接 mock 内部 patch 类型。

推荐保底规则仅限：

1. relation 两端实体名都出现在某个 chunk 文本中。
2. relation 描述文本非空时，描述的规范化结果必须是 chunk 文本规范化结果的子串。
3. 命中的 chunk 必须属于当前窗口。
4. 若多个 chunk 同时命中，则全部保留，不做句级打分和二选一。

初版 deterministic 归因只允许使用以下规范化：
...
- `allowDeterministicAttributionFallback`，默认 `false`
...
4. 若某个 chunk 将接收一条 relation patch，而该 chunk 合并后的 `ExtractionResult` 中缺少关系两端实体，则 merge 阶段必须补齐最小实体：
   - `new ExtractedEntity(name, \"\", \"\", List.of())`

这条规则虽然与当前 `GraphAssembler.ensureEntity(...)` 的兜底行为一致，但 refinement pipeline 不应依赖隐式补点；应在 patch merge 阶段主动闭合端点。
...
4. 最小实体注入发生在“当前 chunk 的所有 patch 合并完成后”，并按 `name + aliases` 规范化去重后再写入最终 `ExtractionResult`
...
1. 已存在关系：
   - `description` 仅在 primary 为空时补
   - `weight` 取 `max(primary, refined)`
2. 不存在关系：新增
...
5. `ChunkExtractionPatch` 若包含 relation，则 merge 后必须保证关系两端实体存在于同一 chunk 的最终 `ExtractionResult` 中；缺失时注入最小实体。
...
- refinement pipeline 应主动补齐 patch 端点实体，而不是依赖 assembler 的隐式补点。
...
5. 分发到某个 chunk 的 relation patch，必须让该 chunk 的最终 `ExtractionResult` 端点闭合。
...
推荐具体落位到 `io.github.lightrag.indexing.refinement` 子包，避免继续膨胀 `indexing` 根包。
...
8. `allowDeterministicAttributionFallback`，默认 `false`

### Cost and Backpressure Rules

1. refinement 不新建线程池，直接运行在 `IndexingPipeline` 现有文档级计算任务内。
2. 对单个文档而言，refinement 在 `computeDocument(...)` 内串行执行，避免一个文档内部多个 anchor 并发放大 LLM 调用。
3. 文档之间仍沿用 `IndexingPipeline.maxParallelInsert` 控制的外层并发。
4. 每文档达到 `maxRefinementPerDocument` 后，剩余 chunk 一律跳过 refinement。
5. 窗口 token 超限时，不升级窗口，只保留一级窗口。
6. 若系统处于高负载，可通过配置直接关闭 refinement。

### Token Estimation Rule

初版不引入新的 tokenizer 依赖，统一沿用项目内现有近似 token 估算方式：

1. 对窗口文本使用与 `KnowledgeExtractor` 一致的近似 token 估算逻辑。
2. `estimatedTokens` 是近似值，不要求和模型 tokenizer 完全一致。
3. 初版窗口裁剪只按 chunk 级进行，不做句子级精剪。
4. “优先保留含候选实体重叠的句子”从本 spec 中降级为后续增强，不进入 Stage 1 实施范围。
...
- 验证无归因 relation 被丢弃
- 验证 relation patch 分发后自动补齐最小端点实体
...
### 5. Test Harness Constraints

1. golden tests 使用固定 fixture JSON，不允许真实 LLM 调用。
2. `ContextualExtractionRefiner` 组件测试必须通过 stub / fake extractor 注入窗口级结果。
3. 断言至少覆盖：
   - supporting chunk ids
   - per-chunk patch 内容
   - 最终 graph 的 `sourceChunkIds`
4. refinement 相关单测默认超时不超过 60s。
...
2. 明确回滚阈值与热切换配置，初始建议：
   - 端到端 ingest P95 增幅超过 25%
   - 每文档额外窗口抽取次数超过配置上限仍频繁命中
   - relation 增量异常偏高且误连边抽查率超过 5%
3. `enabled` 和 `allowDeterministicAttributionFallback` 都必须支持热切换。
4. 回滚观测口径初版建议：
   - 基线：启用前最近 24h 的同类文档 ingest 指标
   - 观测窗口：启用后连续 24h
   - 误连边抽查：每日至少抽样 50 条新增 relation，人工核验
5. 评估是否对外开放外部分片配套 metadata 约束。
6. 评估是否为不同文档类型增加定制 detector 规则。
